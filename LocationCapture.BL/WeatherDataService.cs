...
namespace LocationCapture.BL
{
    public class WeatherDataService : IWeatherDataService
    {
        ...
        private WeatherData ParseWeatherData(string rawWeatherData)
        {
            ...
            result.Temperature = double.Parse(temperatureElement?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);
            result.TemperatureMin = double.Parse(temperatureElement?.Attribute("min")?.Value ?? double.NaN.ToString(), formatProvider);
            result.TemperatureMax = double.Parse(temperatureElement?.Attribute("max")?.Value ?? double.NaN.ToString(), formatProvider);
            result.Humidity = double.Parse(root?.Element("humidity")?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);
            result.Pressure = double.Parse(root?.Element("pressure")?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);
            var windSpeedElement = root?.Element("wind")?.Element("speed");
            var windSpeed = double.Parse(windSpeedElement?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);
            ...
            result.WindDirection = double.Parse(windDirectionElement?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);
            result.WindDirectionName = windDirectionElement?.Attribute("name")?.Value;
            var cloudsElement = root?.Element("clouds");
            result.Cloudiness = double.Parse(cloudsElement?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);
            result.CloudinessName = cloudsElement?.Attribute("name")?.Value;
            var precipitationElement = root?.Element("precipitation");
            var precipitation = double.Parse(precipitationElement?.Attribute("value")?.Value ?? double.NaN.ToString(), formatProvider);            
